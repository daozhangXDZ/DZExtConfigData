...
void USIOJLibrary::ConverFilePath(const FString & FilePath, ESIONFolderType dirctionType, FString & outPath)
{
	FString StartPath;
	switch (dirctionType)
	{
		case ESIONFolderType::Content: StartPath = FPaths::ProjectContentDir(); break;
		case ESIONFolderType::Project: StartPath = FPaths::ProjectDir(); break;
		case ESIONFolderType::Absolute: StartPath = ""; break;
		default: StartPath = FPaths::ProjectDir(); break;
	}
	FString FullJsonPath = FPaths::ConvertRelativePathToFull(StartPath / FilePath);
	outPath = FullJsonPath;
}
